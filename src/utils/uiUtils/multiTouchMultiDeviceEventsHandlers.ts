  ...
  targets.forEach((el: HTMLElement) => {
    ...
    if (isDesktop) {
      if (delayedHandlers.onGestureStart || delayedHandlers.onGestureChange || delayedHandlers.onGestureEnd) {
        const onGestureStart = delayedHandlers.onGestureStart || noop
        const onGestureChange = delayedHandlers.onGestureChange || noop
        const onGestureEnd = delayedHandlers.onGestureEnd || noop
